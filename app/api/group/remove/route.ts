...
export async function POST(req: NextRequest): Promise<NextResponse<Ok | Err>> {
  try {
    ...
    if (!code) return NextResponse.json({ ok: false, message: await apiMsg("noActiveGroup") }, { status: 400 });
    ...
    if (!group) return NextResponse.json({ ok: false, message: await apiMsg("groupNotFound") }, { status: 404 });
    if (group.createdBy !== me) {
      return NextResponse.json({ ok: false, message: await apiMsg("ownerOnlyRemove") }, { status: 403 });
    }
    if (userId === me) {
      return NextResponse.json({ ok: false, message: await apiMsg("useLeave") }, { status: 400 });
    }
    ...
    if (!member) return NextResponse.json({ ok: false, message: await apiMsg("notGroupMember") }, { status: 404 });
